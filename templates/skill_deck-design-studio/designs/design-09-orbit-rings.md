---
name: design-09-orbit-rings
description: "발표 덱 디자인 #09: Orbit Rings. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 09으로', 'Orbit Rings로'처럼 지정하면 이 디자인으로 덱을 만든다. 어두운 배경 기반의 다크 스타일."
---

# 발표 덱 디자인 #09: Orbit Rings
...
## 본문 디자인: Orbit Rings (다크)

**Mood:** 비전, 궤도, 시스템 전경.

**Palette:** 지배 `0B1838`; **악센트 바이올렛→블루 `7C5CFF`→`2E6BE6`**; 보조 희미한 도트 그리드; 텍스트 흰색.

**Fonts:** Calibri. 제목 38pt.

**Background:** `0B1838` (표지 글로브 호응).

**Motif (반복 요소):** **동심 궤도 링 + 하나의 초점 노드**: 링이 한 코너에 정박, 콘텐츠는 반대편.

**Layout:** 우측 궤도, 좌측 콘텐츠.

**Do:** 동심 링, 단일 초점 노드, 차분한 네이비.

**Don't:** 꽉 찬 그래픽, 다중 색, 두꺼운 외곽선.

```javascript
slide.background = { color: "0B1838" };
[[4.2,"2E6BE6"],[3.0,"4F46E5"],[1.8,"7C5CFF"]].forEach(([d,c])=>
  slide.addShape(pres.shapes.OVAL,{ x:9.6-d/2, y:3.75-d/2, w:d, h:d, fill:{ type:"none" }, line:{ color:c, width:1.2, transparency:30 } }));
slide.addShape(pres.shapes.OVAL,{ x:9.45, y:3.6, w:0.3, h:0.3, fill:{ color:"7C5CFF" } });
slide.addText("One platform,\nmany orbits", { x:0.7, y:2.6, w:7, h:1.8, fontFace:"Calibri", bold:true, fontSize:38, color:"FFFFFF", lineSpacing:42, margin:0 });
```
