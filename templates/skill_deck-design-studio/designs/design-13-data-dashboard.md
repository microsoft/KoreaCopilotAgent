---
name: design-13-data-dashboard
description: "발표 덱 디자인 #13: Data Dashboard. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 13으로', 'Data Dashboard로'처럼 지정하면 이 디자인으로 덱을 만든다. 밝은 배경 기반의 라이트 스타일."
---

# 발표 덱 디자인 #13: Data Dashboard
...
## 본문 디자인: Data Dashboard (라이트)

**Mood:** 분석적, 엔지니어링, KPI.

**Palette:** **지배 slate `1E293B`**; 보조 teal `0D9488`, amber `F59E0B`; 배경 흰색; 텍스트 `1E293B`.

**Fonts:** Calibri. 제목 30pt.

**Background:** 흰색 + 슬레이트 헤더 밴드.

**Motif (반복 요소):** **네이티브 편집 가능 차트 + 60~72pt 스탯 콜아웃.**

**Layout:** KPI 밴드 + 차트 + 사이드 패널.

**Do:** 네이티브 차트, 큰 스탯, 슬레이트/틸/앰버.

**Don't:** 이미지로 렌더한 차트, 5색 이상, 장식 컬러바.

```javascript
slide.background = { color: "FFFFFF" };
slide.addShape(pres.shapes.RECTANGLE,{ x:0, y:0.95, w:13.333, h:0.9, fill:{ color:"1E293B" }, line:{type:"none"} });
slide.addText("Performance", { x:0.6, y:1.08, w:9, h:0.7, fontFace:"Calibri", bold:true, fontSize:30, color:"FFFFFF", margin:0 });
[["20","designs","0D9488"],["1","cover","F59E0B"],["100%","on-brand","1E293B"]].forEach(([v,l,c],i)=>{
  slide.addShape(pres.shapes.ROUNDED_RECTANGLE,{ x:0.6+i*2.75, y:2.2, w:2.55, h:1.7, rectRadius:0.08, fill:{ color:"F1F5F9" } });
  slide.addText(v,{ x:0.8+i*2.75, y:2.35, w:2.2, h:1.0, fontFace:"Calibri", bold:true, fontSize:50, color:c, margin:0 }); });
```
