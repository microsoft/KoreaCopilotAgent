---
name: design-20-editorial-tech
description: "발표 덱 디자인 #20: Editorial Tech. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 20으로', 'Editorial Tech로'처럼 지정하면 이 디자인으로 덱을 만든다. 밝은 배경 기반의 라이트 스타일."
---

# 발표 덱 디자인 #20: Editorial Tech
...
## 본문 디자인: Editorial Tech (라이트)

**Mood:** 에디토리얼, 권위, 테크 매거진(세리프 아님).

**Palette:** 지배 잉크 `1A1F2B` on warm-white `FBFBFD`; **악센트 일렉트릭 블루 `1E5BE6`**; 텍스트 `1A1F2B`.

**Fonts:** Cambria(헤더) + Calibri(본문). 제목 40pt.

**Background:** `FBFBFD` (따뜻한 화이트).

**Motif (반복 요소):** **초대형 풀-스탯 + 얇은 컬럼 룰**: 클린 테크 에디토리얼 (옛 세리프 잡지 아님).

**Layout:** 리드 컬럼 + 큰 스탯 + 룰로 나뉜 서브컬럼.

**Do:** 큰 풀-스탯, 얇은 컬럼 룰, 한 점 블루.

**Don't:** 드롭캡 세리프 잡지풍, 따뜻한 색, 장식.

```javascript
slide.background = { color: "FBFBFD" };
slide.addText("The shift", { x:0.6, y:0.6, w:9, h:0.9, fontFace:"Cambria", bold:true, fontSize:40, color:"1A1F2B", margin:0 });
slide.addText("3.5×", { x:0.6, y:1.8, w:4, h:1.6, fontFace:"Cambria", bold:true, fontSize:84, color:"1E5BE6", margin:0 });
slide.addShape(pres.shapes.LINE,{ x:5.0, y:1.9, w:0, h:3.4, line:{ color:"D7DEE8", width:1 } });
slide.addText("핵심 근거를 우측 컬럼에 좌측 정렬로 배치.", { x:5.4, y:1.9, w:7, h:2, fontFace:"Calibri", fontSize:15, color:"3A4150", margin:0 });
```
