---
name: design-06-teal-systems
description: "발표 덱 디자인 #06: Teal Systems. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 06으로', 'Teal Systems로'처럼 지정하면 이 디자인으로 덱을 만든다. 밝은 배경 기반의 라이트 스타일."
---

# 발표 덱 디자인 #06: Teal Systems
...
## 본문 디자인: Teal Systems (라이트)

**Mood:** 클린, 청록, 프로세스/시스템.

**Palette:** **지배 teal `0E7C86`**; 보조 emerald `1FA98F`; 라이트 `E6F4F3`; 배경 흰색 `FFFFFF`; 텍스트 `12343A`.

**Fonts:** Calibri. 제목 36pt.

**Background:** 흰색 (청록 악센트, 표지의 파랑과 다른 변주).

**Motif (반복 요소):** **청록 노드 + 연결 플로우**: 흰 배경에 teal 라운드 노드가 얇은 선으로 이어진 프로세스 플로우. 글자와 겹치지 않게 노드는 하단/측면에 배치.

**Layout:** 좌측 제목 + 하단 플로우 노드.

**Do:** 청록 단일 지배, 흰 배경, 또렷한 텍스트, 노드는 빈 공간에.

**Don't:** 다중 색, 그래픽을 글자 위로 겹치기, 저대비 텍스트.

```javascript
slide.background = { color: "FFFFFF" };
slide.addText("How it flows", { x:0.6, y:0.6, w:9, h:0.8, fontFace:"Calibri", bold:true, fontSize:36, color:"12343A", margin:0 });
const nodes=[["일관성",0.8],["재현성",5.0],["확산",9.2]];
nodes.forEach(([t,x],i)=>{ if(i>0) slide.addShape(pres.shapes.LINE,{ x:x-1.0, y:4.0, w:1.0, h:0, line:{ color:"1FA98F", width:1.5 } });
  slide.addShape(pres.shapes.ROUNDED_RECTANGLE,{ x, y:3.4, w:3.2, h:1.2, rectRadius:0.3, fill:{ color:"E6F4F3" }, line:{ color:"0E7C86", width:1 } });
  slide.addText(t,{ x, y:3.7, w:3.2, h:0.6, fontFace:"Calibri", bold:true, fontSize:18, color:"0E7C86", align:"center", margin:0 }); });
```
