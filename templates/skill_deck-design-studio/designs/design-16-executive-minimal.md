---
name: design-16-executive-minimal
description: "발표 덱 디자인 #16: Executive Minimal. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 16으로', 'Executive Minimal로'처럼 지정하면 이 디자인으로 덱을 만든다. 밝은 배경 기반의 라이트 스타일."
---

# 발표 덱 디자인 #16: Executive Minimal
...
## 본문 디자인: Executive Minimal (라이트)

**Mood:** 임원, 키노트, 절제된 권위.

**Palette:** 지배 charcoal `222630` on near-white `F4F6FA`; **악센트 일렉트릭 블루 `1E5BE6` (한 번)**; 텍스트 `222630`.

**Fonts:** Cambria(제목) + Calibri. 제목 40pt.

**Background:** `F4F6FA` (차분한 오프화이트).

**Motif (반복 요소):** **초대형 숫자 하나 + 헤어라인 룰 하나 + 광활한 여백.**

**Layout:** 좌측 거대 스탯, 희소한 텍스트.

**Do:** 큰 숫자, 한 줄 헤어라인, 넉넉한 여백.

**Don't:** 여러 악센트, 꽉 찬 카드, 장식선.

```javascript
slide.background = { color: "F4F6FA" };
slide.addText("87%", { x:0.7, y:1.6, w:6, h:2.4, fontFace:"Cambria", bold:true, fontSize:130, color:"222630", margin:0 });
slide.addShape(pres.shapes.RECTANGLE,{ x:0.78, y:4.2, w:1.2, h:0.03, fill:{ color:"1E5BE6" } });
slide.addText("of teams reuse the skill weekly", { x:0.78, y:4.5, w:8, h:0.6, fontFace:"Calibri", fontSize:18, color:"4A5161", margin:0 });
```
