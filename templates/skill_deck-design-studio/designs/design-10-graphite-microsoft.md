---
name: design-10-graphite-microsoft
description: "발표 덱 디자인 #10 — Graphite (Microsoft). 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 10으로', 'Graphite (Microsoft)로'처럼 지정하면 이 디자인으로 덱을 만든다. 라이트 — 밝은 배경 기반."
---

# 발표 덱 디자인 #10 — Graphite (Microsoft)
...
## 본문 디자인 — Graphite (Microsoft) (라이트)

**Mood:** Microsoft 뉴트럴, 그래파이트, 절제된 전문성.

**Palette:** **지배 graphite `3B3F46`**; 표면 `F3F4F6`; stroke `E2E4E8`; **악센트 teal `1B9E8A`**; 텍스트 `24262B`.

**Fonts:** Calibri(Segoe 대체). 제목 34pt.

**Background:** 흰색 + 그래파이트 세그먼트 헤더 밴드.

**Motif (반복 요소):** **그래파이트 세그먼트 헤더 + 뉴트럴 카드** (Microsoft 스타일, 회색 기반). 한 카드만 teal 강조.

**Layout:** 상단 그래파이트 밴드 + 회색 뉴트럴 카드 행.

**Do:** 그래파이트 뉴트럴, 한 점 teal, 깔끔한 카드.

**Don't:** 컬러 남발, 진한 그림자, 파란색 일색.

```javascript
const soft = () => ({ type:"outer", color:"3B3F46", blur:8, offset:2, angle:90, opacity:0.10 });
slide.background = { color: "FFFFFF" };
slide.addShape(pres.shapes.RECTANGLE,{ x:0, y:0, w:13.333, h:1.5, fill:{ color:"3B3F46" }, line:{type:"none"} });
slide.addText("Microsoft-style clarity", { x:0.6, y:0.45, w:10, h:0.7, fontFace:"Calibri", bold:true, fontSize:34, color:"FFFFFF", margin:0 });
[["F3F4F6","24262B"],["F3F4F6","24262B"],["1B9E8A","FFFFFF"]].forEach(([c,tc],i)=>{
  slide.addShape(pres.shapes.ROUNDED_RECTANGLE,{ x:0.6+i*4.0, y:2.2, w:3.85, h:3.4, rectRadius:0.06, fill:{ color:c }, line:{ color:"E2E4E8", width:1 }, shadow:soft() });
  slide.addText("0"+(i+1),{ x:0.85+i*4.0, y:2.45, w:3, h:0.8, fontFace:"Calibri", bold:true, fontSize:30, color:(c=="1B9E8A"?"FFFFFF":"1B9E8A"), margin:0 }); });
```
