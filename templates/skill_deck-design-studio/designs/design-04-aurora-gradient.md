---
name: design-04-aurora-gradient
description: "발표 덱 디자인 #04: Aurora Gradient. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 04으로', 'Aurora Gradient로'처럼 지정하면 이 디자인으로 덱을 만든다. 어두운 배경 기반의 다크 스타일."
---

# 발표 덱 디자인 #04: Aurora Gradient
...
## 본문 디자인: Aurora Gradient (다크)

**Mood:** 대기감, 미래, 비전.

**Palette:** 지배 navy `0B1838` → 바이올렛 `3A1E6B` 오로라 + 마젠타 틴트; 텍스트 흰색; 악센트 흰색 하이라이트.

**Fonts:** Calibri. 제목 40pt.

**Background:** **풀블리드 오로라 그라데이션 wash** + 하나의 희미한 대각 라이트 스트릭.

**Motif (반복 요소):** 부드러운 오로라 wash; 텍스트는 wash의 차분한(좌측) 영역에 둠.

**Layout:** 좌측 텍스트, 우측은 빛이 흐르는 오로라.

**Do:** 부드러운 색 전이, 한 줄기 라이트 스트릭, 넓은 호흡.

**Don't:** 딱딱한 도형 경계, 다중 악센트, 꽉 찬 레이아웃.

```javascript
slide.background = { fill:"0B1838" };
slide.addShape(pres.shapes.RECTANGLE,{ x:0, y:0, w:13.333, h:7.5, fill:{ type:"gradient", stops:[
  {color:"0B1838",position:0},{color:"3A1E6B",position:60},{color:"6E4BD8",position:100}], angle:25 }, line:{type:"none"} });
slide.addText("The future, together", { x:0.8, y:2.4, w:7.4, h:1.4, fontFace:"Calibri", bold:true, fontSize:40, color:"FFFFFF", margin:0 });
slide.addText("Connect · Innovate · Empower", { x:0.82, y:3.9, w:7, h:0.6, fontFace:"Calibri", fontSize:17, color:"D8C8FF", margin:0 });
```
