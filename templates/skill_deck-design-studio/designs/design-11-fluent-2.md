---
name: design-11-fluent-2
description: "발표 덱 디자인 #11 — Fluent 2 (Microsoft). 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 11으로', 'Fluent 2 (Microsoft)로'처럼 지정하면 이 디자인으로 덱을 만든다. 라이트 — 밝은 배경 기반."
---

# 발표 덱 디자인 #11 — Fluent 2 (Microsoft)
...
## 본문 디자인 — Fluent 2 (Microsoft) (라이트)

**Mood:** 클린, 레이어드, 엔터프라이즈 신뢰.

**Palette:** **지배 Fluent 블루 `0F6CBD`**; 보조 light `2899F5`, stroke `E1DFDD`; 배경 `FAF9F8`; 텍스트 `242424`.

**Fonts:** Calibri(Segoe 대체, QA-safe). 제목 36pt semibold.

**Background:** `FAF9F8` (Fluent 뉴트럴, 순백 아님).

**Motif (반복 요소):** **중립 베이스 위 떠 있는 흰 카드 + 균일한 depth shadow** (Fluent 깊이 언어). 둥근 모서리 ~8px.

**Layout:** card-on-canvas. 흰 콘텐츠 면이 균일한 그림자로 부유.

**Do:** 절제된 블루, 일관된 elevation, 뉴트럴 베이스.

**Don't:** 시끄러운 그라데이션, 두 종류 이상 블루, 엣지 스트라이프.

```javascript
const depth = () => ({ type:"outer", color:"000000", blur:9, offset:2, angle:90, opacity:0.10 });
slide.background = { color: "FAF9F8" };
slide.addText("Build once, design many", { x:0.6, y:0.5, w:9, h:0.8, fontFace:"Calibri", bold:true, fontSize:36, color:"242424", margin:0 });
[["FFFFFF"],["FFFFFF"],["0F6CBD"]].forEach((c,i)=> slide.addShape(pres.shapes.ROUNDED_RECTANGLE,{ x:0.6+i*3.0, y:1.7, w:2.85, h:3.2, rectRadius:0.08, fill:{ color:c[0] }, shadow:depth() }));
```
