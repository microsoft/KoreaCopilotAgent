---
name: design-07-neon-streams
description: "발표 덱 디자인 #07: Neon Streams. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 07으로', 'Neon Streams로'처럼 지정하면 이 디자인으로 덱을 만든다. 어두운 배경 기반의 다크 스타일."
---

# 발표 덱 디자인 #07: Neon Streams
...
## 본문 디자인: Neon Streams (다크)

**Mood:** 하이에너지, 미래 테크.

**Palette:** 지배 `0D0221`; **악센트 일렉트릭 블루 `2E6BE6` + 바이올렛 `8A50D8` + 마젠타 `EE5091`** (표지 광선색); 텍스트 흰색.

**Fonts:** Arial. 제목 38pt.

**Background:** `0D0221` (표지 광선과 직접 호응).

**Motif (반복 요소):** **네온 글로우 스트릭 라인**: 얇은 선 + 컬러 글로우 섀도. 시선을 이끄는 곡선.

**Layout:** 화면을 가로지르는 스트릭, 콘텐츠는 그 옆.

**Do:** 네온 글로우, 표지 광선색, 다크 베이스.

**Don't:** 파스텔, 무광 플랫, 빨강·초록.

```javascript
const neon = (c) => ({ type:"outer", color:c, blur:18, offset:0, opacity:0.6, angle:0 });
slide.background = { color: "0D0221" };
[["2E6BE6",1.5],["8A50D8",1.9],["EE5091",2.3]].forEach(([c,y])=>
  slide.addShape(pres.shapes.LINE,{ x:0.6, y, w:8.4, h:1.1, line:{ color:c, width:2 }, shadow:neon(c) }));
slide.addText("Momentum", { x:0.6, y:4.6, w:8, h:1, fontFace:"Arial", bold:true, fontSize:38, color:"FFFFFF", margin:0 });
```
