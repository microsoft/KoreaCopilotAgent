---
name: design-18-split-feature
description: "발표 덱 디자인 #18: Split-Feature. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 18으로', 'Split-Feature로'처럼 지정하면 이 디자인으로 덱을 만든다. 밝은 배경 기반의 라이트 스타일."
---

# 발표 덱 디자인 #18: Split-Feature
...
## 본문 디자인: Split-Feature (라이트)

**Mood:** 강한 대비, 포커스, 모던.

**Palette:** **지배 네이비 블록 `0B1838` (풀하이트 40%)** + 흰 콘텐츠; 악센트 앰버 `F2A93B`; 텍스트 흰/잉크.

**Fonts:** Calibri. 제목 38pt.

**Background:** 흰색 + 좌측 풀하이트 네이비 컬럼.

**Motif (반복 요소):** **풀하이트 실제 네이비 사이드 컬럼**(얇은 띠가 아니라 진짜 레이아웃 블록)이 제목을 담음.

**Layout:** 수직 분할: 네이비 제목 영역 / 흰 본문 영역.

**Do:** 풀하이트 컬러 블록, 한 점 앰버, 명료한 분할.

**Don't:** 얇은 장식 스트라이프, 다중 분할, 그라데이션.

```javascript
slide.background = { color: "FFFFFF" };
slide.addShape(pres.shapes.RECTANGLE,{ x:0, y:0, w:5.0, h:7.5, fill:{ color:"0B1838" }, line:{type:"none"} });
slide.addText("Why it\nmatters", { x:0.6, y:2.4, w:3.9, h:2, fontFace:"Calibri", bold:true, fontSize:38, color:"FFFFFF", lineSpacing:42, margin:0 });
slide.addShape(pres.shapes.RECTANGLE,{ x:0.64, y:4.3, w:1.0, h:0.04, fill:{ color:"F2A93B" } });
slide.addText("핵심 메시지를 우측에 좌측 정렬로.", { x:5.5, y:2.6, w:7, h:1.5, fontFace:"Calibri", fontSize:16, color:"1F2330", margin:0 });
```
