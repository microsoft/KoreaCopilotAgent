---
name: design-03-glass-panels
description: "발표 덱 디자인 #03: Glass Panels. 타이틀(표지)부터 본문까지 이 스타일의 팔레트·폰트·모티프·레이아웃을 정확히 따른다. 사용자가 'design 03으로', 'Glass Panels로'처럼 지정하면 이 디자인으로 덱을 만든다. 어두운 배경 기반의 다크 스타일."
---

# 발표 덱 디자인 #03: Glass Panels
...
## 본문 디자인: Glass Panels (다크)

**Mood:** 프리미엄, 모던, 반투명 깊이.

**Palette:** 지배 deep navy `0B1838`; **악센트 바이올렛 `7C5CFF` glow**; 보조 cyan `33C9E8`; 텍스트 흰색.

**Fonts:** Calibri. 제목 38pt.

**Background:** 네이비 그라데이션 `0B1838`→`14224A` + 은은한 컬러 오브.

**Motif (반복 요소):** **프로스티드 글래스 패널**: 반투명 흰 fill(~14%) + 얇은 바이올렛 스트로크. 뒤에 컬러 오브가 비침.

**Layout:** 오브 위에 떠 있는 글래스 카드들.

**Do:** 반투명 패널, 부드러운 바이올렛 글로우, 깊이감.

**Don't:** 불투명 하드 카드, 컬러바, 강한 검정 외곽선.

```javascript
slide.background = { color: "0B1838" };
[["7C5CFF",8.6,1.3,3.0],["33C9E8",9.6,3.6,2.6]].forEach(([c,x,y,d])=>
  slide.addShape(pres.shapes.OVAL,{ x, y, w:d, h:d, fill:{ color:c, transparency:55 }, line:{ type:"none" } }));
slide.addShape(pres.shapes.ROUNDED_RECTANGLE,{ x:0.7, y:1.4, w:11.9, h:5.2, rectRadius:0.12, fill:{ color:"FFFFFF", transparency:88 }, line:{ color:"7C5CFF", width:1 } });
slide.addText("Frosted depth", { x:1.1, y:1.8, w:10, h:0.9, fontFace:"Calibri", bold:true, fontSize:38, color:"FFFFFF", margin:0 });
```
